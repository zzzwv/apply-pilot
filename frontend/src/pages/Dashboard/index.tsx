...
export function DashboardPage() {
  ...
  return <section className="dashboard-page">
    ...
    <Row gutter={[16, 16]} className="dashboard-metrics">
      <Col xs={12} md={8} lg={4}><MetricCard label="总投递" value={data?.total ?? 0} loading={summary.isLoading} icon={<FileTextOutlined />} /></Col>
      <Col xs={12} md={8} lg={4}><MetricCard label="进行中" value={data?.in_progress ?? 0} loading={summary.isLoading} icon={<RiseOutlined />} /></Col>
      <Col xs={12} md={8} lg={4}><MetricCard label="Offer" value={data?.offer_count ?? 0} loading={summary.isLoading} icon={<TrophyOutlined />} /></Col>
      <Col xs={12} md={8} lg={4}><MetricCard label="Offer 获取率" value={data?.offer_rate ?? 0} loading={summary.isLoading} percent icon={<TrophyOutlined />} /></Col>
      <Col xs={12} md={8} lg={4}><MetricCard label="面试通过率" value={data?.interview_rate ?? 0} loading={summary.isLoading} percent icon={<CheckCircleOutlined />} /></Col>
      <Col xs={12} md={8} lg={4}><MetricCard label="淘汰率" value={data?.rejection_rate ?? 0} loading={summary.isLoading} percent icon={<CloseCircleOutlined />} /></Col>
    </Row>
    <Card title="筛选条件" className="dashboard-filters">
      <div className="dashboard-filter-grid">
        ...
        <div className="dashboard-filter-field"><span>投递状态</span><Select allowClear mode="multiple" placeholder="投递状态" options={Object.entries(statusLabels).map(([value, label]) => ({ value, label }))} value={filters.status} onChange={(value: ApplicationStatus[]) => updateFilters({ status: value.length ? value : undefined })} /></div>
        <div className="dashboard-filter-field"><span>企业性质</span><Select allowClear mode="multiple" placeholder="企业性质" options={natureOptions.map(([value, label]) => ({ value, label }))} value={filters.company_nature} onChange={(value: string[]) => updateFilters({ company_nature: value.length ? value : undefined })} /></div>
        <div className="dashboard-filter-field"><span>投递类型</span><Select allowClear mode="multiple" placeholder="投递类型" options={Object.entries(applicationTypeLabels).map(([value, label]) => ({ value, label }))} value={filters.application_type} onChange={(value: ApplicationType[]) => updateFilters({ application_type: value.length ? value : undefined })} /></div>
        <div className="dashboard-filter-field"><span>行业</span><Select allowClear mode="tags" placeholder="行业" value={filters.industry} onChange={(value: string[]) => updateFilters({ industry: value.length ? value : undefined })} /></div>
        <div className="dashboard-filter-field"><span>企业规模</span><Select allowClear mode="multiple" placeholder="企业规模" options={sizeOptions.map((value) => ({ value, label: value }))} value={filters.company_size} onChange={(value: string[]) => updateFilters({ company_size: value.length ? value : undefined })} /></div>
        <div className="dashboard-filter-field"><span>投递日期</span><DatePicker.RangePicker value={filters.date_from && filters.date_to ? [dayjs(filters.date_from), dayjs(filters.date_to)] : undefined} onChange={(value) => updateFilters({ date_from: value?.[0]?.format("YYYY-MM-DD"), date_to: value?.[1]?.format("YYYY-MM-DD") })} /></div>
