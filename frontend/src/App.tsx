...
export function App({ queryClient }: Props) {
  return <ConfigProvider><AuthBootstrap /><Layout style={{ minHeight: "100vh", padding: 32 }}><Space style={{ width: "100%", justifyContent: "space-between" }} align="start"><Typography.Title>秋招 / 实习投递管理</Typography.Title>{queryClient && <AuthControls queryClient={queryClient} />}</Space><nav><Link to="/">首页</Link>{" · "}<Link to="/applications">投递记录</Link></nav><Suspense fallback={<Spin />}><Routes><Route path="/" element={<DashboardPage />} /><Route path="/applications" element={<ApplicationsPage />} /><Route path="/applications/:id" element={<ApplicationDetailPage />} /></Routes></Suspense></Layout></ConfigProvider>;
}
