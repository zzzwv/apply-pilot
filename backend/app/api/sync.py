    ...
    for item in payload.applications:
        existing = await session.scalar(
            select(JobApplication).where(
                ...
                JobApplication.client_sync_id == item.client_sync_id,
            )
        )
        if existing:
            reused += 1
            mappings.append(
                {
                    "client_sync_id": str(item.client_sync_id),
                    "cloud_application_id": str(existing.id),
                }
            )
            continue
        try:
            company = await session.scalar(
                select(Company).where(Company.full_name == item.company.full_name)
            )
            if company is None:
            ...
            mappings.append(
                {
                    "client_sync_id": str(item.client_sync_id),
                    "cloud_application_id": str(application.id),
                }
            )
        except Exception:
